...
if ( !class_exists( 'wpRPG' ) ) {
    class wpRPG {
        ...
        public function wpRPG_RegisterSettings( ) {
            ...
            add_option( 'show_wpRPG_Version_footer', "1", "", "yes" );
...
?>
								<option value="<?php
                echo esc_attr( stripslashes( $race->ID ) );
?>" ><?php
                echo $race->title;
?> </option>
							<?php
...
class testit extends wpRPG {
    function __construct( ) {
        parent::__construct();
        add_filter( 'listPlayers_Loggedin_Actions', array(
             $this,
            'add_filterss' 
        ) );
    }
...
if ( is_admin() ) {
    add_action( 'admin_menu', array(
         $rpg,
        'wpRPG_settings_page_init' 
    ) );
    add_action( 'admin_init', array(
         $rpg,
        'wpRPG_RegisterSettings' 
    ) );
}
if ( !is_admin() ) {
